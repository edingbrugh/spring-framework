...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	 ...
	 * @param inputSource用于从
	 * @param resource读取XML文件的资源描述符@返回DOM文档的SAX inputSource
	 */
	...
	/**
	 * 确定指定的{@link资源}的验证模式。如果没有配置显式验证模式，则验证模式从给定的资源获取{@link detectValidationMode detected}。
	 * 如果你想要完全控制验证模式，重写此方法，即使设置了{@link VALIDATION_AUTO}以外的内容。@see detectValidationMode
	 */
	protected int getValidationModeForResource(Resource resource) {
		...
		//我们没有得到明确的指示…让我们假设XSD，因为在检测停止之前(在找到文档的根标记之前)显然没有找到DTD声明。
		return VALIDATION_XSD;
	}

	/**
	 * 检测对由提供的{@link资源}标识的XML文件执行哪种类型的验证。如果文件具有{@code DOCTYPE}定义，
	 * 则使用DTD验证，否则假定XSD验证。如果你想自定义{@link VALIDATION_AUTO}模式的分辨率，请覆盖此方法。
	 */
	...
	/**
	 * 注册给定DOM文档中包含的bean定义。由{@code loadBeanDefinitions}调用。<p>创建解析器类的新实例并调用
	...
	/**
	 * 创建{@link BeanDefinitionDocumentReader}用于实际从XML文档读取bean定义。默认实现实例化指定的"documentReaderClass"。
	 */
	...
	/**
	 * 惰性地创建一个默认的NamespaceHandlerResolver，如果之前没有设置。
	 */
	...
	/**
	 * 创建默认的{@link NamespaceHandlerResolver}实现，如果没有指定则使用。默认实现返回{@link DefaultNamespaceHandlerResolver}的实例。
	 */
